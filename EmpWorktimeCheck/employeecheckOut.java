...
public class employeecheckOut extends JFrame {
  ...
  private JLabel lCHECKOUT2 = new JLabel();
  // end attributes
  ...
  public employeecheckOut() { 
    ...
    DataTable();
    lYourID1.setBounds(72, 184, 113, 33);
    lYourID1.setText("Your ID");
    cp.add(lYourID1);
    lTime1.setBounds(72, 232, 113, 33);
    lTime1.setText("Time");
    cp.add(lTime1);
    jTextField1.setBounds(208, 184, 144, 40);
    cp.add(jTextField1);
    jTextField2.setBounds(208, 232, 144, 40);
    cp.add(jTextField2);
    jTextField2.setEnabled(true);
    bCheckOut1.setBounds(144, 392, 264, 40);
    bCheckOut1.setText("Check Out");
    bCheckOut1.setMargin(new Insets(2, 2, 2, 2));
    bCheckOut1.setBackground(Color.decode("#FF6363"));
    Border border = BorderFactory.createLineBorder(Color.decode("#B2B2B2"));        
    bCheckOut1.setBorder(border);
//    
    ...
    cp.add(bEdit1);
    bDelete1.setBounds(232, 464, 80, 24);
    ...
    cp.add(bDelete1);
    bClear1.setBounds(368, 464, 80, 24);
    ...
    cp.add(bClear1);
    jScrollPane1.setBounds(24, 536, 488, 216);
    jScrollPane1.setBorder(border1); 
    cp.add(jScrollPane1);
    bRefreshTable1.setBounds(24, 504, 480, 24);
    ...
    jTextField3.setBounds(360, 184, 112, 40);
    cp.add(jTextField3);
    jTextField3.setEnabled(false);
    
    
    lChangeto1.setBounds(384, 152, 80, 24);
    lChangeto1.setText("Change to ");
    cp.add(lChangeto1);
    jTextField4.setBounds(208, 328, 144, 40);
    cp.add(jTextField4);
    jTextField4.setEnabled(false);
    
    lCheck1.setBounds(72, 328, 113, 33);
    lCheck1.setText("Check");
    cp.add(lCheck1);
    jTextField5.setBounds(208, 280, 144, 40);
    lDate1.setBounds(72, 280, 113, 33);
    cp.add(jTextField5);
    lDate1.setText("Date");
    cp.add(lDate1);
    lCHECKOUT2.setBounds(112, 64, 319, 60);
    lCHECKOUT2.setForeground(Color.decode("#FF6363"));  
    lCHECKOUT2.setText("CHECK OUT !");
    lCHECKOUT2.setFont(new Font("Dialog", Font.BOLD, 48));
    cp.add(lCHECKOUT2);
    // end components
    setVisible(true);
  };
  ...
  public void DataTable(){
    try {
      ...
      ResultSet rs = st.executeQuery("SELECT * FROM emptimecheckout");
  ...
  public void bCheckOut1_ActionPerformed(ActionEvent evt) {
    ...
    try {
      ...
      ps = conn.prepareStatement("SELECT * FROM emptimecheckout WHERE YOURID = ? AND DATE = ?");
      ps.setString(1, YOURID);
      ps.setString(2, DATE);
      rs = ps.executeQuery();
      
      int dialogResult = JOptionPane.showConfirmDialog (null, "Do you really want to check out this ID ","Warning",JOptionPane.YES_NO_OPTION);
      ...
      if (rs.next()) {
        ...
        JOptionPane.showMessageDialog(null, "You have already checked out today.", "Warning", JOptionPane.WARNING_MESSAGE);
      } else {
        // The employee has not checked in today, insert check-in record
        ps = conn.prepareStatement("INSERT INTO emptimecheckout VALUES (?, ?, ?, ?)");
        ps.setString(1, YOURID);
 ...
 // end of bCheckOut1_ActionPerformed
  ...
  public void bEdit1_ActionPerformed(ActionEvent evt) {
    ...
    try {
      ...
      ps = conn.prepareStatement("update emptimecheckout set YOURID =? where YOURID= ? ");
  ...
  public void bDelete1_ActionPerformed(ActionEvent evt) {
    ...
    try {
      ...
      if(dialogResult == JOptionPane.YES_OPTION){
        ...
        ps = conn.prepareStatement("delete from emptimecheckout where YOURID=? ");
        ps.setString(1, YOURID);
